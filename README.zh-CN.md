...
2. 安装 Web Cap 浏览器扩展：
   ...
   - 开启开发者模式。
   - 将解压后的扩展文件夹拖到扩展程序页面中。
   - 打开 Web Cap 扩展详情，开启 `Allow User Scripts`。
...
### 在 Hacker News 上复用 Web Cap Hub 脚本

运行 [web-cap-hub](https://github.com/edgestorage/web-cap-hub) 中的可复用脚本，以更少的页面探索、更少的 token 和更快的速度总结当前 Hacker News 页面前 5 篇帖子的评论内容。

![Web Cap Hacker News 可复用脚本示例](https://raw.githubusercontent.com/edgestorage/web-cap-docs/main/examples/example-hn.gif)

### 用一句话隐藏 YouTube 内容区块

用一句话隐藏 YouTube Gaming 页面里的 `Top live games` 内容区块，并让它在后续访问中持续生效。

![Web Cap YouTube userscript 示例](https://raw.githubusercontent.com/edgestorage/web-cap-docs/main/examples/example-userscript-youtube.gif)
...
- 面向真实 Chromium 系浏览器标签页的浏览器扩展运行时。
...
Web Cap 的核心价值不是让 agent 每次都从零开始操作浏览器，而是把已经验证过的浏览器操作沉淀为可复用脚本和可复用工作流。

对于重复出现的页面和任务，agent 可以直接复用稳定流程，减少反复理解页面、规划步骤、确认按钮和纠错的过程。这可以提升操作准确性和执行效率，同时降低重复浏览器探索带来的 token 消耗和时间消耗。

因此，Web Cap 更适合作为 Codex、Claude Code 或其他本地 agent 工具的浏览器能力层：让大模型专注于目标理解和决策，把稳定、可重复的浏览器操作交给本地自动化流程完成。
...
这份路线图概述 Web Cap 和 Web Cap Hub 后续计划推进的主要方向。
...
降低对 Node.js / npm 环境的依赖，探索更简单的安装、构建与分发方式。

### 浏览器端 AI Chat 界面与本地 AI 工具集成

提供浏览器内的 AI Chat 操作入口，并连接本地 Codex、Claude Code 等工具完成实际执行。

### 脚本编译职责下沉到客户端

将较重的 TypeScript 编译相关能力从浏览器插件迁移到客户端，降低插件体积和复杂度。
...
- 当前运行时主要在 Chromium 系浏览器上验证；Firefox 兼容仍属于计划中的工作。
